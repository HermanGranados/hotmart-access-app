...
import { LIBRES, DESTACADA, PREMIUM_COMPACTAS } from "@/lib/herramientas";
        ...
        <div className="min-h-screen bg-white">
          <div className="max-w-md mx-auto bg-white min-h-screen sm:border-x border-slate-100">
            ...
            <div className="px-4 pt-6 pb-4 space-y-6">
              {/* ── Herramientas Premium — desde lib/herramientas.ts ── */}
              <div>
                ...
                {DESTACADA && (
                  <TarjetaDestacada
                    h={DESTACADA}
                    bloqueada={!isPremium}
                    onClick={() => { if (isPremium) setVistaActual(DESTACADA!.id as typeof vistaActual); }}
                  />
                )}
                ...
                {PREMIUM_COMPACTAS.map((h) => (
                  <TarjetaPremium
                    key={h.id}
                    h={h}
                    onClick={() => { if (isPremium) setVistaActual(h.id as typeof vistaActual); }}
                  />
                ))}
              </div>

              {/* ── Herramientas Incluidas — desde lib/herramientas.ts ── */}
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-400 px-1 mb-4">
                ...
                <div className="space-y-3">
                  {LIBRES.map((h) => (
                    <TarjetaLibre
                      ...
                      h={h}
                      badge="Incluida"
                      onClick={() => setVistaActual(h.id as typeof vistaActual)}
                    />
                  ))}
                </div>
              </div>
